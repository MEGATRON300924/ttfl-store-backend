...
export async function recordEvent(campaignId: string, eventType: string, visitorKey?: string, metadata?: unknown) {
  await ensureAdTables(); if (!EVENTS.includes(eventType)) throw AppError.badRequest("Invalid ad event");
  const active = await prisma.$queryRawUnsafe<any[]>(`SELECT id FROM ad_campaigns WHERE id=$1 AND status='ACTIVE' AND start_at<=NOW() AND end_at>NOW() LIMIT 1`, campaignId); if (!active[0]) throw AppError.badRequest("Ad campaign is not active", "AD_NOT_ACTIVE");
  ...
  await prisma.$executeRawUnsafe(`INSERT INTO ad_events (id,campaign_id,event_type,visitor_key,metadata) VALUES ($1,$2,$3,$4,$5::jsonb)`, randomUUID(), campaignId, eventType, visitorKey ?? null, JSON.stringify(metadata ?? {})); return { recorded: true };
}
...
export async function getCampaignAnalytics(userId: string, campaignId: string) {
  await ensureAdTables(); const vendor = await getVendorProfileForUser(userId); const campaign = await getCampaignForVendor(campaignId, vendor.id);
  const rows = await prisma.$queryRawUnsafe<Array<{ event_type: string; count: number; revenue: number }>>(`SELECT event_type,COUNT(*)::int AS count,COALESCE(SUM(CASE WHEN event_type IN ('PURCHASE','BOOKING') THEN COALESCE((metadata->>'amount')::numeric,0) ELSE 0 END),0)::float AS revenue FROM ad_events WHERE campaign_id=$1 GROUP BY event_type`, campaignId);
