...
adsRouter.post("/campaigns/:id/boost", requireAuth, requireRole("VENDOR"), asyncHandler(async(req,res)=>{ const user=await prisma.user.findUniqueOrThrow({where:{id:req.user!.sub},select:{email:true}}); res.json(await ads.boostCampaign(req.user!.sub,req.params.id,user.email)); }));
...
adsRouter.get("/admin/moderation", requireAuth, requireRole("ADMIN"), asyncHandler(async(_req,res)=>{ res.json({ campaigns: await ads.listPendingModeration() }); }));
adsRouter.post("/admin/moderation/:id", requireAuth, requireRole("ADMIN"), asyncHandler(async(req,res)=>{ const action=z.enum(["APPROVE","REJECT"]).parse(req.body.action); res.json(await ads.moderateCampaign(req.params.id,action)); }));
