...
vendorPlansRouter.get("/", asyncHandler(async (_req, res) => { res.json({ plans: await vendorPlansService.listPlans() }); }));
...
vendorPlansRouter.put("/:tier", requireAuth, requireRole("ADMIN"), asyncHandler(async (req, res) => {
  const tier = req.params.tier as "FREE" | "PRO" | "BUSINESS" | "ENTERPRISE";
  if (!["FREE", "PRO", "BUSINESS", "ENTERPRISE"].includes(tier)) return res.status(400).json({ error: { code: "INVALID_TIER", message: "Unknown tier" } });
