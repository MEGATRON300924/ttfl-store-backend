...
reviewsRouter.get("/store/:storeSlug", asyncHandler(async (req, res) => {
  const page = Math.max(1, Number(req.query.page ?? 1));
  const limit = Math.min(50, Math.max(1, Number(req.query.limit ?? 20)));
  const result = await reviewsService.getStoreReviews(req.params.storeSlug, page, limit);
  res.json(result);
}));
...
reviewsRouter.post("/:id/report", requireAuth, asyncHandler(async (req, res) => {
...
reviewsRouter.post("/admin/:id/hide", requireAuth, requireRole("ADMIN"), asyncHandler(async (req, res) => {
  const review = await reviewsService.adminHideReview(req.params.id, req.user!.sub);
  res.json({ review });
}));

reviewsRouter.post("/admin/:id/restore", requireAuth, requireRole("ADMIN"), asyncHandler(async (req, res) => {
  const review = await reviewsService.adminRestoreReview(req.params.id);
  res.json({ review });
}));

reviewsRouter.delete("/admin/:id", requireAuth, requireRole("ADMIN"), asyncHandler(async (req, res) => {
