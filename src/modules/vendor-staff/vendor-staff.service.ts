...
import { STAFF_ROLE_PERMISSIONS, type VendorPermission, type VendorStaffRole, requireVendorOwner } from "@/lib/vendor-access";
...
export async function listStaff(ownerId: string) {
  ...
  const rows = await prisma.vendorStaff.findMany({
...
export async function inviteStaff(ownerId: string, input: { email: string; role: string }) {
  ...
  const owner = await prisma.vendorProfile.findUnique({ where: { id: membership.vendorId }, select: { storeName: true } });
  if (!owner) throw AppError.notFound("Vendor profile not found");
  ...
  await prisma.vendorStaffInvitation.updateMany({
    where: { vendorId: membership.vendorId, email, acceptedAt: null },
  ...
  void sendEmail({ to: email, ...vendorStaffInvitationEmail(owner.storeName, role, inviteUrl) });
...
export async function getInvitation(rawToken: string) {
  ...
  const invitation = await prisma.vendorStaffInvitation.findUnique({ where: { tokenHash } });
...
export async function acceptInvitation(rawToken: string, input: { firstName?: string; lastName?: string; password?: string }, userId?: string) {
  ...
  const safeUser = { id: user.id, email: user.email, role: user.role, status: user.status, firstName: user.firstName, lastName: user.lastName, phone: user.phone, avatarUrl: user.avatarUrl, emailVerified: user.emailVerified };
  const accessToken = signAccessToken({ sub: user.id, role: user.role, emailVerified: user.emailVerified });
  return { user: safeUser, accessToken, staff };
}
