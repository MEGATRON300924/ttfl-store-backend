...
export async function ensureAffiliateTables() {
  await prisma.$executeRawUnsafe(`
    CREATE TABLE IF NOT EXISTS affiliates (
      ...
      code TEXT NOT NULL UNIQUE, status TEXT NOT NULL DEFAULT 'ACTIVE', commission_rate NUMERIC(5,2) NOT NULL DEFAULT 5,
...
export async function join(userId: string) {
  ...
  const existing = await prisma.$queryRawUnsafe<Array<{ id: string; code: string; status: string; commission_rate: string }>>(`SELECT id, code, status, commission_rate::text FROM affiliates WHERE user_id = $1 LIMIT 1`, userId);
  ...
  await prisma.$executeRawUnsafe(`INSERT INTO affiliates (id, user_id, code, commission_rate) VALUES ($1, $2, $3, $4)`, id, userId, code, DEFAULT_RATE);
  return { id, code, status: "ACTIVE", commission_rate: String(DEFAULT_RATE) };
}

export async function getDashboard(userId: string) {
  await ensureAffiliateTables();
  const rows = await prisma.$queryRawUnsafe<Array<{ id: string; code: string; status: string; commission_rate: string; clicks: number; conversions: number; pending_earnings: string; paid_earnings: string; created_at: Date }>>(`SELECT id, code, status, commission_rate::text, clicks, conversions, pending_earnings::text, paid_earnings::text, created_at FROM affiliates WHERE user_id = $1 LIMIT 1`, userId);
  const affiliate = rows[0];
  if (!affiliate) return null;
  const commissions = await prisma.$queryRawUnsafe<Array<{ id: string; order_id: string; order_number: string; order_amount: string; amount: string; status: string; created_at: Date; paid_at: Date | null }>>(`SELECT c.id, c.order_id, o.order_number, c.order_amount::text, c.amount::text, c.status, c.created_at, c.paid_at FROM affiliate_commissions c JOIN orders o ON o.id = c.order_id WHERE c.affiliate_id = $1 ORDER BY c.created_at DESC LIMIT 50`, affiliate.id);
  return {
    affiliate: { id: affiliate.id, code: affiliate.code, status: affiliate.status, commissionRate: Number(affiliate.commission_rate), clicks: affiliate.clicks, conversions: affiliate.conversions, pendingEarnings: Number(affiliate.pending_earnings), paidEarnings: Number(affiliate.paid_earnings), createdAt: affiliate.created_at },
    commissions: commissions.map((c) => ({ id: c.id, orderId: c.order_id, orderNumber: c.order_number, orderAmount: Number(c.order_amount), amount: Number(c.amount), status: c.status, createdAt: c.created_at, paidAt: c.paid_at })),
  };
}

export async function recordClick(code: string, input: { sessionId?: string; landingPath?: string; source?: string }) {
  await ensureAffiliateTables();
  const rows = await prisma.$queryRawUnsafe<Array<{ id: string }>>(`SELECT id FROM affiliates WHERE code = $1 AND status = 'ACTIVE' LIMIT 1`, code.trim().toUpperCase());
  if (!rows[0]) return { tracked: false };
  const id = randomBytes(16).toString("hex");
  await prisma.$executeRawUnsafe(`INSERT INTO affiliate_clicks (id, affiliate_id, session_id, landing_path, source) VALUES ($1, $2, $3, $4, $5)`, id, rows[0].id, input.sessionId ?? null, input.landingPath ?? null, input.source ?? null);
...
export async function resolveAttribution(code: string | undefined, customerId: string) {
  if (!code) return null;
  await ensureAffiliateTables();
  const rows = await prisma.$queryRawUnsafe<Array<{ id: string; user_id: string; commission_rate: string }>>(`SELECT id, user_id, commission_rate::text FROM affiliates WHERE code = $1 AND status = 'ACTIVE' LIMIT 1`, code.trim().toUpperCase());
...
export async function attachOrder(orderId: string, affiliateId: string, commissionRate: number) {
  await ensureAffiliateTables();
  await prisma.$executeRawUnsafe(`INSERT INTO affiliate_attributions (id, affiliate_id, order_id, commission_rate) VALUES ($1, $2, $3, $4) ON CONFLICT (order_id) DO NOTHING`, randomBytes(16).toString("hex"), affiliateId, orderId, commissionRate);
}
