...
import { deleteStoreBranding } from "@/modules/uploads/uploads.service";
import * as service from "./store-profile.service";
import { uploadStoreGalleryImage } from "@/modules/uploads/uploads.service";
...
storeProfileRouter.post("/me/gallery", requireAuth, requireRole("VENDOR"), upload.single("image"), asyncHandler(async (req, res) => {
  ...
  res.status(201).json({ image: await service.addGalleryImage(req.user!.sub, result.url, result.publicId) });
}));
