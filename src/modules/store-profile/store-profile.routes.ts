...
storeProfileRouter.get("/public/directory", asyncHandler(async (req, res) => {
  await service.ensureStoreProfileTables(); await ensureStoreCategoryTable();
  const limit=Math.min(Math.max(Number(req.query.limit??24)||24,1),48); const page=Math.max(Number(req.query.page??1)||1,1); const offset=(page-1)*limit; const search=typeof req.query.q==="string"?req.query.q.trim():""; const verifiedOnly=String(req.query.verified??"").toLowerCase()==="true";
  const rows=await prisma.$queryRawUnsafe<any[]>(`SELECT vp.id,vp."storeName" AS "name",vp."storeSlug" AS "slug",vp."logoUrl" AS "logoUrl",vp.location,vp.verified,vp.tier,spp.custom_url AS "customUrl",COALESCE(spp.visibility,'PUBLIC') AS visibility,vsc.category_slug AS "categorySlug",c.name AS "categoryName",COUNT(DISTINCT p.id)::int AS "productCount",COALESCE(AVG(p.avg_rating),0)::float AS rating FROM vendor_profiles vp LEFT JOIN store_public_profiles spp ON spp.vendor_id=vp.id LEFT JOIN vendor_store_categories vsc ON vsc.vendor_id=vp.id LEFT JOIN categories c ON c.slug=vsc.category_slug LEFT JOIN products p ON p.vendor_id=vp.id AND p.status='ACTIVE' AND p.deleted_at IS NULL WHERE vp.status='APPROVED' AND COALESCE(spp.visibility,'PUBLIC')<>'PRIVATE' AND ($1='' OR vp."storeName" ILIKE '%'||$1||'%' OR vp.location ILIKE '%'||$1||'%' OR c.name ILIKE '%'||$1||'%') AND ($2=false OR vp.verified=true) GROUP BY vp.id,spp.custom_url,spp.visibility,vsc.category_slug,c.name ORDER BY vp."storeName" ASC LIMIT $3 OFFSET $4`,search,verifiedOnly,limit,offset);
  const countRows=await prisma.$queryRawUnsafe<any[]>(`SELECT COUNT(DISTINCT vp.id)::int AS count FROM vendor_profiles vp LEFT JOIN store_public_profiles spp ON spp.vendor_id=vp.id LEFT JOIN vendor_store_categories vsc ON vsc.vendor_id=vp.id LEFT JOIN categories c ON c.slug=vsc.category_slug WHERE vp.status='APPROVED' AND COALESCE(spp.visibility,'PUBLIC')<>'PRIVATE' AND ($1='' OR vp."storeName" ILIKE '%'||$1||'%' OR vp.location ILIKE '%'||$1||'%' OR c.name ILIKE '%'||$1||'%') AND ($2=false OR vp.verified=true)`,search,verifiedOnly);
  const ids=rows.map(row=>row.id); const badges=ids.length?await prisma.$queryRawUnsafe<any[]>(`SELECT vendor_id,badge FROM store_badges WHERE vendor_id=ANY($1::text[]) ORDER BY created_at ASC`,ids):[]; const badgeMap=new Map<string,string[]>(); for(const row of badges) badgeMap.set(row.vendor_id,[...(badgeMap.get(row.vendor_id)??[]),row.badge]); const total=Number(countRows[0]?.count??0);
  res.json({stores:rows.map(row=>({...row,productCount:Number(row.productCount),rating:Number(row.rating),badges:badgeMap.get(row.id)??(row.verified?["VERIFIED"]:[])})),pagination:{page,limit,total,totalPages:Math.ceil(total/limit)}});
}));
...
const profileSchema=z.object({headline:z.string().trim().max(160).nullable().optional(),description:z.string().trim().max(3000).nullable().optional(),theme:z.enum(["CLASSIC","DARK","MINIMAL"]).optional(),accentColor:z.string().regex(/^#[0-9a-fA-F]{6}$/).optional(),layout:z.enum(["STANDARD","EDITORIAL","CATALOG"]).optional(),customUrl:z.string().trim().max(80).nullable().optional(),visibility:z.enum(["PUBLIC","PRIVATE","DISPLAY"]).optional()});
