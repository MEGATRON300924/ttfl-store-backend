...
async function generateOrderNumber(): Promise<string> { const year = new Date().getFullYear(); let orderNumber: string; do { orderNumber = `TTFL-${year}-${Math.floor(100000 + Math.random() * 900000)}`; } while (await prisma.order.findUnique({ where: { orderNumber } })); return orderNumber; }

async function createOrderSplitCode(order: { orderNumber: string; totalAmount: unknown; vendorOrders: Array<{ vendorId: string; vendorEarnings: unknown }> }): Promise<string | undefined> { if (!order.vendorOrders.length) return undefined; const vendorIds = order.vendorOrders.map((item) => item.vendorId); const vendors = await prisma.vendorProfile.findMany({ where: { id: { in: vendorIds } }, select: { id: true, storeName: true, paystackSubaccountCode: true } }); const byVendorId = new Map(vendors.map((vendor) => [vendor.id, vendor])); const missing = order.vendorOrders.find((item) => !byVendorId.get(item.vendorId)?.paystackSubaccountCode); if (missing) { const vendor = byVendorId.get(missing.vendorId); logger.warn("Skipping Paystack split because a vendor payout account is not configured", { orderNumber: order.orderNumber, vendor: vendor?.storeName ?? missing.vendorId }); return undefined; } const totalKobo = Math.round(Number(order.totalAmount) * 100); if (!Number.isFinite(totalKobo) || totalKobo <= 0) return undefined; const shares = order.vendorOrders.map((item) => ({ subaccount: byVendorId.get(item.vendorId)!.paystackSubaccountCode!, share: Math.max(0, Math.round(Number(item.vendorEarnings) * 100)) })).filter((item) => item.share > 0); if (!shares.length) return undefined; const requestedShareTotal = shares.reduce((sum, item) => sum + item.share, 0); if (requestedShareTotal > totalKobo) { const scale = totalKobo / requestedShareTotal; for (const item of shares) item.share = Math.floor(item.share * scale); } const finalShares = shares.filter((item) => item.share > 0); if (!finalShares.length) return undefined; try { return await createTransactionSplit({ name: `TTFL ${order.orderNumber}`, type: "flat", bearerType: "account", subaccounts: finalShares }); } catch (error) { logger.warn("Paystack split creation failed; continuing checkout without split", { orderNumber: order.orderNumber, error }); return undefined; } }
...
export async function checkout(customerId: string, customerEmail: string, input: CheckoutInput) {
  ...
  const groups = new Map<string, { items: { product: Product; quantity: number; unitPrice: number; variantKey?: string; variantLabel?: string; variantOptions?: Record<string, string> }[] }>();

  for (const line of input.items) {
    const product = byId.get(line.productId);
    if (!product) throw AppError.badRequest(`Product ${line.productId} is no longer available`, "PRODUCT_UNAVAILABLE");
    if (product.status !== "ACTIVE" || product.comingSoon) throw AppError.badRequest(`"${product.name}" is not currently available`, "PRODUCT_UNAVAILABLE");
    if (product.sellingMethod !== "CHECKOUT") throw AppError.badRequest(`"${product.name}" is sold outside TTFL Store checkout — use the vendor's link/WhatsApp instead`, "WRONG_SELLING_METHOD");
    const deal = deals.get(product.id);
    if (product.stock < line.quantity) throw AppError.badRequest(`Not enough stock for "${product.name}"`, "INSUFFICIENT_STOCK");
    if (deal && deal.quantityCap !== null && deal.soldCount + deal.reservedCount + line.quantity > deal.quantityCap) throw AppError.badRequest(`The Flash Deal for "${product.name}" has reached its remaining quantity`, "FLASH_DEAL_CAP_REACHED");

    const resolved = resolveCheckoutUnitPrice(product, line.variantKey, deal?.salePrice);
    if (line.variantKey && !resolved.variant) throw AppError.badRequest(`The selected variation for "${product.name}" is no longer available`, "VARIANT_UNAVAILABLE");
    if (resolved.unitPrice === null || !Number.isFinite(resolved.unitPrice) || resolved.unitPrice < 0) throw AppError.badRequest(`The price for "${product.name}" is invalid`, "INVALID_PRODUCT_PRICE");

    const group = groups.get(product.vendorId) ?? { items: [] };
    group.items.push({ product, quantity: line.quantity, unitPrice: resolved.unitPrice, variantKey: resolved.variant?.key, variantLabel: resolved.variant?.label, variantOptions: resolved.variant?.options });
  ...
  for (const [vendorId, group] of groups) {
    ...
    vendorOrderData.push({ vendor: { connect: { id: vendorId } }, subtotal, commissionRate: rate, commissionAmount, vendorEarnings, items: { create: group.items.map((item) => ({ productId: item.product.id, productName: item.variantLabel ? `${item.product.name} — ${item.variantLabel}` : item.product.name, unitPrice: item.unitPrice, quantity: item.quantity, lineTotal: item.unitPrice * item.quantity, variantKey: item.variantKey, variantLabel: item.variantLabel, variantOptions: item.variantOptions as Prisma.InputJsonValue | undefined })) } });
  }
...
export async function verifyAndFinalizePayment(reference: string) {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    if (current?.paymentStatus === "PAID") { alreadyFinalized = true; return; }
  ...
  for (const vendorOrder of order.vendorOrders) { const vendor = await prisma.vendorProfile.findUnique({ where: { id: vendorOrder.vendorId }, include: { user: true } }); if (vendor) { void sendEmail({ to: vendor.user.email, ...vendorNewOrderEmail(order.orderNumber, vendorOrder.items.length) }); void createNotification(vendor.user.id,{title:"New order",body:`You have a new order: ${order.orderNumber}.`,type:"VENDOR_ORDER",url:`/vendor/dashboard`}).catch(()=>undefined); } }
...
export async function getOrderByNumber(orderNumber: string, requesterId: string, requesterRole: string) { const order = await prisma.order.findUnique({ where: { orderNumber }, include: { vendorOrders: { include: { items: true, vendor: true } } } }); if (!order) throw AppError.notFound("Order not found"); const isOwner = order.customerId === requesterId; const isVendorOnOrder = order.vendorOrders.some((vendorOrder) => vendorOrder.vendor.userId === requesterId); if (!isOwner && !isVendorOnOrder && requesterRole !== "ADMIN") throw AppError.forbidden("You don't have access to this order"); return order; }
...
export async function updateVendorOrderStatus(userId: string, vendorOrderId: string, nextStatus: OrderStatus) { const vendor = await prisma.vendorProfile.findUniqueOrThrow({ where: { userId } }); const vendorOrder = await prisma.vendorOrder.findUnique({ where: { id: vendorOrderId } }); if (!vendorOrder || vendorOrder.vendorId !== vendor.id) throw AppError.notFound("Order not found"); if (!FORWARD_TRANSITIONS[vendorOrder.status].includes(nextStatus)) throw AppError.badRequest(`Can't move an order from ${vendorOrder.status} to ${nextStatus}`, "INVALID_STATUS_TRANSITION"); return prisma.vendorOrder.update({ where: { id: vendorOrderId }, data: { status: nextStatus } }); }
export async function refundOrder(orderId: string, adminId: string) { const order = await prisma.order.findUnique({ where: { id: orderId }, include: { vendorOrders: { include: { items: true } } } }); if (!order) throw AppError.notFound("Order not found"); if (order.paymentStatus !== "PAID") throw AppError.badRequest("Only paid orders can be refunded", "ORDER_NOT_PAID"); await refundTransaction(order.paymentReference, Number(order.totalAmount)); await prisma.$transaction(async (tx) => { await tx.order.update({ where: { id: order.id }, data: { paymentStatus: "REFUNDED" } }); await tx.vendorOrder.updateMany({ where: { orderId: order.id }, data: { status: "REFUNDED" } }); for (const vendorOrder of order.vendorOrders) for (const item of vendorOrder.items) await tx.product.update({ where: { id: item.productId }, data: { stock: { increment: item.quantity } } }); }); void reversePurchase(order.id).catch(() => undefined); const customer = await prisma.user.findUnique({ where: { id: order.customerId } }); if (customer) void sendEmail({ to: customer.email, ...orderRefundedEmail(order.orderNumber) }); await recordAudit({ actorId: adminId, action: "ORDER_REFUNDED", targetType: "Order", targetId: order.id }); return prisma.order.findUniqueOrThrow({ where: { id: order.id }, include: { vendorOrders: { include: { items: true } } } }); }
