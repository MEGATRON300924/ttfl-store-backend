...
export async function checkout(customerId: string, customerEmail: string, input: CheckoutInput) {
  ...
  for (const line of input.items) {
    ...
    if (d && d.quantityCap !== null && d.soldCount + line.quantity > d.quantityCap) {
      throw AppError.badRequest(`The Flash Deal for "${p.name}" has reached its remaining quantity`, "FLASH_DEAL_CAP_REACHED");
    }
  ...
  for (const [vendorId, g] of groups) {
    ...
    vendorOrderData.push({
      ...
      vendorEarnings,
      items: {
        create: g.items.map((i) => ({ productId: i.product.id, productName: i.product.name, unitPrice: i.unitPrice, quantity: i.quantity, lineTotal: i.unitPrice * i.quantity })),
      },
    });
  }
...
export async function verifyAndFinalizePayment(reference: string) {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    for (const item of order.vendorOrders.flatMap((vo) => vo.items)) {
      await tx.$executeRawUnsafe(
        `UPDATE flash_deals fd SET sold_count=(SELECT COALESCE(SUM(oi.quantity),0)::int FROM order_items oi JOIN vendor_orders vo ON vo.id=oi.vendor_order_id JOIN orders o ON o.id=vo.order_id WHERE oi.product_id=fd.product_id AND o.payment_status='PAID') WHERE fd.product_id=$1 AND fd.active=true`,
        item.productId,
      );
    }
  });
  ...
  if (customer) void sendEmail({ to: customer.email, ...orderConfirmationEmail(order.orderNumber) });
  for (const vo of order.vendorOrders) {
...
export async function getOrderByNumber(orderNumber: string, requesterId: string, requesterRole: string) {
  ...
  const isVendorOnOrder = order.vendorOrders.some((vo) => vo.vendor.userId === requesterId);
...
export async function updateVendorOrderStatus(userId: string, vendorOrderId: string, nextStatus: OrderStatus) {
  const vendor = await prisma.vendorProfile.findUniqueOrThrow({ where: { userId } });
  const vo = await prisma.vendorOrder.findUnique({ where: { id: vendorOrderId } });
  if (!vo || vo.vendorId !== vendor.id) throw AppError.notFound("Order not found");
  if (!FORWARD_TRANSITIONS[vo.status].includes(nextStatus)) throw AppError.badRequest(`Can't move an order from ${vo.status} to ${nextStatus}`, "INVALID_STATUS_TRANSITION");
