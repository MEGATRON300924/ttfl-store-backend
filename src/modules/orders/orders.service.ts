...
async function createOrderSplitCode(order: { orderNumber: string; totalAmount: unknown; vendorOrders: Array<{ vendorId: string; vendorEarnings: unknown }> }): Promise<string | undefined> {
  if (!order.vendorOrders.length) return undefined;
  ...
  if (missing) {
    ...
    logger.warn("Skipping Paystack split because a vendor payout account is not configured", { orderNumber: order.orderNumber, vendor: vendor?.storeName ?? missing.vendorId });
    return undefined;
  }
  const totalKobo = Math.round(Number(order.totalAmount) * 100);
  if (!Number.isFinite(totalKobo) || totalKobo <= 0) return undefined;
  const shares = order.vendorOrders.map((item) => ({ subaccount: byVendorId.get(item.vendorId)!.paystackSubaccountCode!, share: Math.max(0, Math.round(Number(item.vendorEarnings) * 100)) })).filter((item) => item.share > 0);
  ...
  try {
    return await createTransactionSplit({ name: `TTFL ${order.orderNumber}`, type: "flat", bearerType: "account", subaccounts: finalShares });
  } catch (error) {
    logger.warn("Paystack split creation failed; continuing checkout without split", { orderNumber: order.orderNumber, error });
    return undefined;
  }
}
...
export async function checkout(customerId: string, customerEmail: string, input: CheckoutInput) {
  ...
  const groups = new Map<string, { items: { product: Product; quantity: number; unitPrice: number; variantLabel?: string }[] }>();
  ...
  for (const line of input.items) {
    ...
    group.items.push({ product, quantity: line.quantity, unitPrice: resolved.unitPrice, variantLabel: resolved.variant?.label });
  ...
  for (const [vendorId, group] of groups) {
    ...
    for (const item of group.items) couponLines.push({ vendorId, categoryId: item.product.categoryId, lineTotal: item.unitPrice * item.quantity });
  }
  ...
  for (const [vendorId, group] of groups) {
    const originalSubtotal = group.items.reduce((sum, item) => sum + item.unitPrice * item.quantity, 0);
  ...
  try {
    await prisma.$transaction(async (tx) => {
      await reserveForOrder(tx, order.id, order.vendorOrders.flatMap((vendorOrder) => vendorOrder.items.map((item) => ({ productId: item.productId, quantity: item.quantity }))));
    });
  } catch (error) {
    await releaseForOrder(order.id, "RELEASED");
    throw error;
  }
...
export async function verifyAndFinalizePayment(reference: string) {
  ...
  if (verification.status !== "success") {
    await releaseForOrder(order.id, "FAILED");
  ...
  for (const vendorOrder of order.vendorOrders) {
    const vendor = await prisma.vendorProfile.findUnique({ where: { id: vendorOrder.vendorId }, include: { user: true } });
    if (vendor) void sendEmail({ to: vendor.user.email, ...vendorNewOrderEmail(order.orderNumber, vendorOrder.items.length) });
  }
...
export async function getMyOrders(customerId: string) {
  return prisma.order.findMany({ where: { customerId }, include: { vendorOrders: { include: { items: true } } }, orderBy: { createdAt: "desc" } });
}

export async function getOrderByNumber(orderNumber: string, requesterId: string, requesterRole: string) {
  const order = await prisma.order.findUnique({ where: { orderNumber }, include: { vendorOrders: { include: { items: true, vendor: true } } } });
  if (!order) throw AppError.notFound("Order not found");
  const isOwner = order.customerId === requesterId;
  const isVendorOnOrder = order.vendorOrders.some((vendorOrder) => vendorOrder.vendor.userId === requesterId);
  if (!isOwner && !isVendorOnOrder && requesterRole !== "ADMIN") throw AppError.forbidden("You don't have access to this order");
  return order;
}
...
export async function updateVendorOrderStatus(userId: string, vendorOrderId: string, nextStatus: OrderStatus) {
  ...
  if (!FORWARD_TRANSITIONS[vendorOrder.status].includes(nextStatus)) throw AppError.badRequest(`Can't move an order from ${vendorOrder.status} to ${nextStatus}`, "INVALID_STATUS_TRANSITION");
...
export async function refundOrder(orderId: string, adminId: string) {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    for (const vendorOrder of order.vendorOrders) {
      for (const item of vendorOrder.items) await tx.product.update({ where: { id: item.productId }, data: { stock: { increment: item.quantity } } });
    }
  });
