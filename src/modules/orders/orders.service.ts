...
export async function verifyAndFinalizePayment(reference: string) {
  ...
  // `data.amount` is the gross amount the customer paid (for example ₦152.29
  // for a ₦150 order). Paystack also returns `requested_amount`, which is the
  // original amount requested by TTFL. Verify against that value so Paystack's
  // customer-paid fee does not incorrectly trigger AMOUNT_MISMATCH.
