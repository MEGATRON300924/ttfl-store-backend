...
export async function verifyAndFinalizePaymentSafely(reference: string) {
  ...
  if (Math.round(requestedAmountNaira * 100) !== Math.round(Number(initialOrder.totalAmount) * 100)) {
    throw AppError.badRequest("Payment amount does not match order total", "AMOUNT_MISMATCH");
  }

  // IMPORTANT: record the successful gateway payment separately from fulfillment.
  // A stock/flash-deal/vendor SQL failure must never erase the fact that Paystack
  ...
  // Payment is already safely recorded. Fulfillment is now independently retryable.
  ...
  if (needsFulfillment) {
    try {
      await prisma.$transaction(async tx => {
        ...
        for (const vo of current.vendorOrders) {
          ...
          for (const item of vo.items) {
            ...
            if (updated.count !== 1) {
              throw AppError.badRequest(`Not enough stock for "${item.productName}"`, "INSUFFICIENT_STOCK");
            }
          }
      ...
      logger.error("Paystack payment recorded but TTFL order fulfillment failed; retry is safe", {
        reference,
        orderId: order.id,
        error
      });
