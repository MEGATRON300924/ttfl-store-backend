...
const EXPLANATIONS: Record<string, string> = {
  ...
  INSUFFICIENT_STOCK: "The requested quantity could not be reserved because available stock was insufficient. Check inventory and concurrent orders.",
  INVALID_PRODUCT_PRICE: "The product price changed or did not match the price submitted by the customer. Compare the current product price with the checkout request.",
  ...
  ORDER_NOT_PAID: "The requested order action requires a successful payment, but the order is not marked as paid. Check payment verification and the gateway reference.",
  ORDER_REFUNDED: "The order has already been refunded. Check the refund record before attempting another payment or fulfillment action.",
  ...
  SERVICE_REQUIRES_QUOTE: "This service requires a vendor quote before checkout. Check the service configuration and quote state.",
  INVALID_SERVICE_PRICE: "The service price submitted for the order is invalid or no longer matches the configured price. Check the service and quote.",
  UNSUPPORTED_SERVICE_CURRENCY: "The selected service uses a currency that TTFL Store does not support for this checkout.",
