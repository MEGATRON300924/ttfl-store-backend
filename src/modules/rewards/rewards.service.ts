...
async function award(userId: string, points: number, type: string, description: string, referenceType?: string, referenceId?: string, idempotencyKey?: string) {
  ...
  if (!inserted.length) return false; await prisma.$executeRawUnsafe(`UPDATE reward_wallets SET points_balance=points_balance+$1,lifetime_earned=lifetime_earned+$1,updated_at=NOW() WHERE user_id=$2`, Math.floor(points), userId); await refreshLevel(userId); return true;
}
...
export async function refreshLevel(userId:string){await ensureWallet(userId);const rows=await prisma.$queryRawUnsafe<any[]>(`SELECT lifetime_earned AS "lifetimeEarned", lifetime_spend AS "lifetimeSpend", completed_orders AS "completedOrders" FROM reward_wallets WHERE user_id=$1`,userId);const w=rows[0];const level=levelFor(Number(w.lifetimeEarned),Number(w.lifetimeSpend),Number(w.completedOrders));await prisma.$executeRawUnsafe(`UPDATE reward_wallets SET level=$1,updated_at=NOW() WHERE user_id=$2`,level,userId);return level;}
