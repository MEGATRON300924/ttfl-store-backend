...
export async function getWallet(userId: string) {
  await ensureWallet(userId); await ensureWelcomeReward(userId);
...
export async function redeem(userId:string,points:number,kind:"ORDER_DISCOUNT"|"DELIVERY_DISCOUNT",referenceId?:string){if(!Number.isInteger(points)||points<=0)throw AppError.badRequest("Enter a valid number of reward points","INVALID_REWARD_POINTS");const wallet=await getWallet(userId);if(points>Number(wallet.pointsBalance))throw AppError.badRequest("You do not have enough TTFL Rewards points","INSUFFICIENT_REWARD_POINTS");const id=randomUUID();await prisma.$transaction(async(tx)=>{const updated=await tx.$executeRawUnsafe(`UPDATE reward_wallets SET points_balance=points_balance-$1,lifetime_redeemed=lifetime_redeemed+$1,updated_at=NOW() WHERE user_id=$2 AND points_balance >= $1`,points,userId);if(updated!==1)throw AppError.badRequest("Your reward balance changed. Please try again","REWARD_BALANCE_CHANGED");await tx.$executeRawUnsafe(`INSERT INTO reward_ledger(id,user_id,type,points,description,reference_type,reference_id,created_at) VALUES($1,$2,'REDEEM',-$3,$4,'${kind}',$5,NOW())`,id,userId,points,kind==="ORDER_DISCOUNT"?"TTFL Rewards order discount":"TTFL Rewards delivery discount",referenceId??null);});return{pointsRedeemed:points,nairaValue:points,kind};}
