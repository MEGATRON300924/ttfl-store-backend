...
export async function getWhatsAppAdminNumbers(): Promise<string[]> {
  const value = await getSetting(SETTING_KEYS.WHATSAPP_ADMIN_NUMBERS);
  return value.split(/[,\n]/).map((number) => number.trim()).filter(Boolean).map((number) => number.replace(/\D/g, "")).filter((number) => number.length >= 7);
}

export async function getAllSettings(): Promise<Record<string, string>> {
  const rows = await prisma.platformSetting.findMany({ where: { key: { in: [...EDITABLE_KEYS] } } });
...
export async function setSetting(key: string, value: string) {
  if (!EDITABLE_KEYS.has(key)) throw AppError.badRequest("That platform setting cannot be changed", "SETTING_NOT_EDITABLE");
