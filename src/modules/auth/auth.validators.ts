...
export const registerVendorSchema = registerCustomerSchema.extend({
  ...
  storeCategory: z.string().trim().min(2).max(120),
...
export const updateProfileSchema = z.object({ firstName: z.string().min(1).max(80).optional(), lastName: z.string().min(1).max(80).optional(), phone: z.string().min(7).max(20).optional() });
export const avatarSchema = z.object({ avatarUrl: z.string().url() });
