...
const sellingMethodFields = z.discriminatedUnion("sellingMethod", [
  z.object({
    sellingMethod: z.literal("CHECKOUT"),
  }),
  ...
  z.object({
    ...
    externalUrl: z.string().url(
      "A valid external purchase URL is required"
    ),
  }),
...
export const createProductSchema = z
  ...
  .refine(
    ...
    {
      message:
        "Previous price must be greater than the current price to represent a discount",
      path: ["previousPrice"],
    }
  );
...
export const updateProductSchema = z.object({
  ...
  sellingMethod: z
    .enum(["CHECKOUT", "EXTERNAL_LINK", "WHATSAPP"])
    .optional(),

  externalUrl: optionalString(
    z.string().url("A valid external purchase URL is required")
  ),
