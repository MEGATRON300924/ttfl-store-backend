...
serviceOrdersRouter.post("/", requireAuth, asyncHandler(async (req, res) => {
  ...
  const customer = await import("@/lib/prisma").then(({ prisma }) => prisma.user.findUnique({ where: { id: req.user!.sub }, select: { email: true } }));
  if (!customer) return res.status(404).json({ error: { message: "Account not found" } });
...
serviceOrdersRouter.post("/:reference/verify", requireAuth, asyncHandler(async (req, res) => {
  ...
  if (serviceOrder.customer_id !== req.user!.sub) throw new Error("Unauthorized");
