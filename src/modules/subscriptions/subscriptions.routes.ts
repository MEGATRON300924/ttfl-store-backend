...
subscriptionsRouter.post("/resume", requireAuth, requireRole("VENDOR"), asyncHandler(async (req, res) => { const vendor = await getVendorProfileForUser(req.user!.sub); res.json(await subscriptionsService.resumeSubscription(vendor.id)); }));
