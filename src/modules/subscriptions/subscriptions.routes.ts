...
subscriptionsRouter.post("/cancel", requireAuth, requireRole("VENDOR"), asyncHandler(async (req, res) => { const vendor = await getVendorProfileForUser(req.user!.sub); const user = await prisma.user.findUniqueOrThrow({ where: { id: req.user!.sub } }); res.json(await subscriptionsService.cancelSubscription(vendor.id, user.email)); }));
subscriptionsRouter.post("/resume", requireAuth, requireRole("VENDOR"), asyncHandler(async (req, res) => { const vendor = await getVendorProfileForUser(req.user!.sub); const user = await prisma.user.findUniqueOrThrow({ where: { id: req.user!.sub } }); res.json(await subscriptionsService.resumeSubscription(vendor.id, user.email)); }));
