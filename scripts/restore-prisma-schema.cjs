...
function normalizeProductVendorColumn(schema) {
  ...
  if (productStart === -1) throw new Error("Downloaded Prisma schema is missing model Product.");

  const productEnd = schema.indexOf("\n}", productStart);
  if (productEnd === -1) throw new Error("Downloaded Prisma schema has an invalid Product model.");

  const productBlock = schema.slice(productStart, productEnd);
  const normalized = productBlock
  ...
  if (!normalized.includes('vendorId      String           @map("vendor_id")')) {
    throw new Error("Product vendorId field could not be normalized to the existing vendor_id database column.");
  }

  return schema.slice(0, productStart) + normalized + schema.slice(productEnd);
}

(async () => {
  let schema = await download(sourceUrl);

  if (!schema.includes("model User {") || !schema.includes("model Order {") || !schema.includes("model VendorProfile {") || !schema.includes("model Product {")) {
  ...
  console.log("Canonical Prisma schema restored with the existing products.vendor_id mapping and all local schema extensions applied.");
})().catch((error) => {
